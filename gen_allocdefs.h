...
int ta_ready(x,n) register ta *x; register int n; { register int i; \
 if (x->field) { i = x->a; \
   if (n > i) { x->a = base + n + (n >> 3); \
...
int ta_rplus(x,n) register ta *x; register int n; { register int i; \
 if (x->field) { i = x->a; n += x->len; \
   if (n > i) { x->a = base + n + (n >> 3); \
     if (alloc_re(&x->field,i * sizeof(type),x->a * sizeof(type))) return 1; \
     x->a = i; return 0; } \
   return 1; } \
